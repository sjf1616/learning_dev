def loadsql():
    import csv
    import os
    if os.path.exists('src/data/ex115sql.csv'):
        with open('src/data/ex115sql.csv', 'r',newline='', encoding='utf-8') as file:
            dict_file = csv.DictReader(file)
            for e, r in enumerate(dict_file):
                print(f'{e+1}- {r['nome'].capitalize():<20}{r['idade']} anos')
            print()
    else:
        with open('src/data/ex115sql.csv', 'w',newline='', encoding='utf-8') as file:
...
def insert_sql(name, age):
    import csv
    with open('src/data/ex115sql.csv', 'a', newline='', encoding='utf-8') as file:
